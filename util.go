...
func (s *levelHandler) validate() error {
	...
	for j := 1; j < numTables; j++ {
		...
		if s.tables[j-1].Biggest().Compare(s.tables[j].Smallest()) >= 0 {
			return errors.Errorf(
				"Inter: %q vs %q: level=%d j=%d numTables=%d",
				string(s.tables[j-1].Biggest().UserKey), string(s.tables[j].Smallest().UserKey), s.level, j, numTables)
		}

		if s.tables[j].Smallest().Compare(s.tables[j].Biggest()) > 0 {
			return errors.Errorf(
